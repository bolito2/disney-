...
class RNN:
    ...
    # Inputs:
    # start -> string representing how the name should start

    # Outputs:
    # valid -> Boolean that tells if the input string was valid
    # output -> Word generated by the RNN, if the input was valid
    def gen_name(self, start):
        if len(start) == 0:
            raise ValueError('The RNN has to start with a non-empty string')

        # Encode the input string
        valid, X = one_hot_string(start)

        if not valid:
            return False, None

        # Initialize the hidden values
        a = np.zeros((self.units, 1))

        # Feed-forward the input to make the RNN take into account the input string
        for i in range(X.shape[0]):
            # x has to be a column vector
            x = np.reshape(X[i], [-1, 1])
            # One step of forward propagation(output isn't needed)
            a, _ = self.rnn_cell(x, a)

        # Now that the hidden state accounts for the input, generate new letters
        letter = start[-1]

        # Create the output string, excluding last letter as it will get added again below
        gen_string = start[:-1]

        # When the RNN outputs the end token, stop
        while letter != '>':
            # Add the predicted letter to the output
            gen_string += letter

            # Encode last letter
            x = one_hot_character(letter)

            # Forward-propagate one step
            a, y = self.rnn_cell(x, a)

            # Get the probability of each letter by the softmax layer, and unroll them into a 1-dim array
            probabilities = np.reshape(softmax(y), [-1])

            # Sample the letters and pick one
            letter_index = np.random.choice(n_letters, p=probabilities)
            letter = letters[letter_index]

        # Return the generated string
        return True, gen_string
